  ...
  <article class="data-list-item">
    <div class="data-card-shell">
      ...
      <aside class="action-column">
        <button class="primary-btn" @click.stop="handleOpenSource">
          {{ $t('dataCard.viewData') }}
        </button>
        <button class="ghost-btn" @click.stop="handleDetails">
          {{ $t('dataCard.details') }}
        </button>
      </aside>
    </div>
...
const emit = defineEmits(['view-source', 'details'])
...
const handleOpenSource = () => emit('view-source', props.data)
const handleDetails = () => emit('details', props.data)
</script>
