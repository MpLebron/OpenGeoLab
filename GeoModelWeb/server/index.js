...
const { getDatabase, getDatabaseInfo, initDatabase } = require('./db/database');
...
} = require('./utils/pyGeoModelCatalog');
const {
...
} = require('./services/ogmsModelSyncService');
const {
...
app.use('/api/case-assets', express.static(CASE_LIBRARY_DIR, {
    immutable: true,
    maxAge: '30d'
}));
...
// Application scheme routes read from local MongoDB and sync OpenGMP data on demand.
...
const OGMS_DEPLOYED_MODEL_QUERY_TOKEN = process.env.OGMS_DEPLOYED_MODEL_QUERY_TOKEN || '';
...
const OGMS_MODEL_LIST_SOURCE = (process.env.OGMS_MODEL_LIST_SOURCE || 'db').toLowerCase();
const OGMS_MODEL_DB_SCOPE = (process.env.OGMS_MODEL_DB_SCOPE || 'pygeomodel').toLowerCase();
...
const DATA_CENTER_LIST_CACHE_TTL_MS = parseInt(process.env.DATA_CENTER_LIST_CACHE_TTL_MS || '300000', 10);
const DATA_CENTER_SOURCE_LIST_PAGE_SIZE = parseInt(process.env.DATA_CENTER_SOURCE_LIST_PAGE_SIZE || '1000', 10);
const dataCenterThumbnailCache = new Map();
const dataCenterListCache = new Map();
...
async function enrichDataCenterItem(item) {
    ...
    const sourceUrl = resolveDataCenterSourceUrl(item, { webHost: DATA_CENTER_WEB_HOST });
...
async function fetchLinkableDataCenterContent(params) {
    const cacheKey = buildDataCenterListCacheKey(params);
    const cachedValue = getOgmsCacheValue(dataCenterListCache, cacheKey);
    if (cachedValue) {
        return cachedValue;
    }

    const sourcePageSize = Math.max(1, Number.parseInt(DATA_CENTER_SOURCE_LIST_PAGE_SIZE, 10) || 1000);
    const firstPageParams = {
        ...params,
        page: 1,
        pageSize: sourcePageSize
    };
    const firstResponse = await opengmpApi.post('/centerRes/getResourceDataList', firstPageParams);
    const firstData = firstResponse.data?.data || {};
    const allContent = Array.isArray(firstData.content) ? [...firstData.content] : [];

    if (firstResponse.data?.code !== 0) {
        return {
            responseData: firstResponse.data,
            content: []
        };
    }

    const totalPages = Math.max(1, Number.parseInt(firstData.totalPages, 10) || 1);
    if (totalPages > 1) {
        const pages = Array.from({ length: totalPages - 1 }, (_, index) => index + 2);
        const pageResponses = await mapInBatches(pages, 4, page => (
            opengmpApi.post('/centerRes/getResourceDataList', {
                ...firstPageParams,
                page
            })
        ));
        ...
        for (const pageResponse of pageResponses) {
            if (pageResponse.data?.code !== 0) {
                throw new Error(pageResponse.data?.msg || 'Failed to fetch a data center page');
            }

            const pageContent = pageResponse.data?.data?.content;
    ...
    const content = allContent.filter(item => hasDataCenterSourceLink(item, { webHost: DATA_CENTER_WEB_HOST }));
    const payload = {
        responseData: {
            ...firstResponse.data,
            data: {
    ...
    setOgmsCacheValue(dataCenterListCache, cacheKey, payload, DATA_CENTER_LIST_CACHE_TTL_MS);
    return payload;
}
...
function mapDataMethodSummary(item) {
    ...
    const tags = rawTags
        .map(tag => DATA_METHOD_TAG_NAME_MAP[String(tag)] || String(tag))
        .map(normalizeMethodFacetLabel)
        .filter(isDisplayableMethodFacetLabel);
    ...
    return {
        ...
        engine: normalizeDataMethodEngine(item.uuid || item.engine || item.environment || item.language),
        execution: normalizeMethodFacetLabel(item.execution) || null,
        methodType: normalizeMethodFacetLabel(item.type) || null,
        category: normalizeMethodFacetLabel(item.category) || null,
...
function isDisplayableMethodFacetLabel(label) {
    const normalized = normalizeMethodFacetLabel(label);
...
function normalizeDataMethodEngine(value) {
...
function matchesMethodFilters(method, {
    ...
    const methodType = String(method.methodType || '').toLowerCase();
    ...
    if (python && !(
        engine.includes('python') ||
        engine === 'py' ||
        execution.includes('python') ||
        execution === 'py' ||
        methodType.includes('python') ||
        methodType === 'py'
    )) {
        return false;
    }
...
app.post('/api/datacenter/list', async (req, res) => {
    try {
        ...
        const requestedPageSize = Math.max(1, Number.parseInt(req.body.pageSize, 10) || 18);
        const params = {
            asc: req.body.asc === true || req.body.asc === 'true',
        ...
        console.log('[DataCenter] Fetching linkable data list:', {
            ...params,
            page: requestedPage,
            pageSize: requestedPageSize
        });

        const { responseData, content } = await fetchLinkableDataCenterContent(params);

        if (responseData.code === 0) {
            const pageData = paginateDataCenterContent(
                content,
                requestedPage,
                requestedPageSize,
                { webHost: DATA_CENTER_WEB_HOST }
            );
            const enrichedContent = pageData.content.length > 0
                ? await Promise.all(pageData.content.map(item => enrichDataCenterItem(item)))
                : [];
            ...
            res.status(400).json({
                code: -1,
                msg: responseData.msg || 'Failed to fetch data'
            });
        }
    } catch (error) {
...
function mapOgmsModelSummary(item, detail = null) {
    return normalizeOpenGmsModelFavorite({
        ...
        md5: item.md5 || null,
        relatedModelItems: detail?.relatedModelItems || item.relatedModelItems || []
    });
}
...
function mergeOgmsModelListPayloads(primary, fallback, {
    page = 1,
    limit = 20,
    source = 'db+pygeomodel'
} = {}) {
    const seen = new Set();
    const merged = [];

    [...(primary?.data || []), ...(fallback?.data || [])].forEach(model => {
        const keys = [
            model.sourceId,
            model.id,
            model.model_id,
    ...
    return {
        ...
        page,
        limit,
        data: merged.slice(startIndex, startIndex + limit)
    };
}
...
async function fetchOgmsModelDetailByMd5(md5) {
    ...
    const mappedDetail = {
        ...
        relatedModelItems: Array.isArray(detail.relatedModelItems) ? detail.relatedModelItems : []
    };
...
// List OGMS Models. Defaults to the local MongoDB OpenGMS index, which is
// refreshed daily from the OpenGMS deployed-model endpoint.
app.get('/api/ogms/models', async (req, res) => {
    try {
        ...
        if (OGMS_MODEL_LIST_SOURCE === 'pygeomodel') {
            return res.json(listPyGeoModelModels({
                query: search,
                page,
                limit,
                domain
        ...
        if (OGMS_MODEL_LIST_SOURCE === 'db') {
            try {
                const db = getDatabase();
                if (await countOgmsModels(db) > 0) {
                    const executableScope = OGMS_MODEL_DB_SCOPE !== 'all';
                    if (executableScope) {
                        const registryFilter = buildPyGeoModelRegistryFilter();
                        const dbPayload = await listOgmsModels(db, {
                            query: search,
                            page: 1,
                        ...
                        const staticPayload = listPyGeoModelModels({
                            query: search,
                            page: 1,
                            limit: Number.MAX_SAFE_INTEGER,
                            domain
                        ...
                        return res.json(mergeOgmsModelListPayloads(
                            dbPayload,
                            staticPayload,
                            { page, limit, source: 'db+pygeomodel' }
                        ));
                    }
                    ...
                    const syncCompleted = await countCompletedOgmsModelSyncRuns(db) > 0;
                    if (search && !syncCompleted) {
                        return res.json(mergeOgmsModelListPayloads(
                            dbPayload,
                            listPyGeoModelModels({
                                query: search,
                ...
                console.warn('MongoDB OGMS model list failed, fallback to PyGeoModel catalog:', dbError.message);
            }
...
app.get('/api/ogms/models/facets', async (req, res) => {
    try {
        ...
        if (OGMS_MODEL_LIST_SOURCE === 'pygeomodel') {
            const scopedModels = listPyGeoModelModels({
                query: search,
                page: 1,
                limit: Number.MAX_SAFE_INTEGER
            });
            return res.json({
                total: scopedModels.total,
                domains: buildOgmsDomainFacetCounts(scopedModels.data).slice(0, 8)
            });
        }
        ...
        if (OGMS_MODEL_LIST_SOURCE === 'db') {
            try {
                ...
                if (await countOgmsModels(db) > 0) {
                    ...
                    return res.json(await getOgmsModelFacets(db, {
                        query: search,
                        online,
                        publicOnly,
                        institutionalOnly
                ...
                console.warn('MongoDB OGMS model facets failed, fallback to PyGeoModel catalog:', dbError.message);
            }
...
async function parseXmlMdlToJSON(xmlContent) {
    return parseOgmsXmlMdlToJSON(xmlContent);
}
...
app.get('/api/ogms/models/:name', async (req, res) => {
    try {
        ...
        if (OGMS_MODEL_LIST_SOURCE === 'pygeomodel') {
            const localModelInfo = getPyGeoModelModelDetail(name);
            if (localModelInfo) {
                return res.json(localModelInfo);
            }
        }
        ...
        if (OGMS_MODEL_LIST_SOURCE === 'db') {
            try {
                const db = getDatabase();
                if (await countOgmsModels(db) > 0) {
                    const dbModelInfo = await findOgmsModelDetail(db, name);
                    if (dbModelInfo) {
                        return res.json(dbModelInfo);
                    }
                }
            } catch (dbError) {
                console.warn(`MongoDB OGMS model detail failed for ${name}, fallback to PyGeoModel catalog:`, dbError.message);
            }
        ...
        if (response.data && response.data.data) {
            ...
            if (modelInfo.mdl && typeof modelInfo.mdl === 'string') {
                try {
                    ...
                    const parsedMdl = await parseOgmsXmlMdlToJSON(modelInfo.mdl);
                    modelInfo.mdl = parsedMdl; // Replace string with parsed object
                    modelInfo.mdlJson = parsedMdl;
                    modelInfo.normalizedInputs = parsedMdl.inputs || [];
                    modelInfo.parameterParseStatus = 'parsed';
                } catch (e) {
                    console.warn(`Failed to parse XML MDL for ${name}, returning original.`);
                    modelInfo.parameterParseStatus = 'failed';
                    modelInfo.syncError = `MDL parse failed: ${e.message}`;
                }
            }
...
async function startServer() {
    ...
    const ogmsModelSyncService = createOgmsModelSyncService({
        db,
        httpClient: ogmsApi,
        deployedModelUrl: OGMS_DEPLOYED_MODEL_URL,
        logger: console
    });
    startOgmsModelDailySync({
        countModels: () => countOgmsModels(db),
        syncService: ogmsModelSyncService,
        logger: console
    });
