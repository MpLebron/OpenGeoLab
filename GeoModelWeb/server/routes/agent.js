...
# 创建模型访问实例 (使用实际的模型名称)
token = os.getenv("OGMS_TOKEN")
if not token:
    raise RuntimeError("Set OGMS_TOKEN before invoking OpenGMS models.")
model = OGMSAccess("模型名称", token=token)
...
**重要**: 不要在 Notebook 中硬编码 Token。请从运行环境的 `OGMS_TOKEN` 读取凭证。
