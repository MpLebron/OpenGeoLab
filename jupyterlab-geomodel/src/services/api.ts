...
if (typeof window !== 'undefined') {
  ...
  if (projectFromUrl) {
    localStorage.setItem('geomodel_project', projectFromUrl);
  }
  if (tokenFromUrl) {
...
export async function resolveProjectName(): Promise<string> {
  if (typeof window === 'undefined') {
    return '';
  }
  ...
  if (projectFromContainer) {
    localStorage.setItem('geomodel_project', projectFromContainer);
    return projectFromContainer;
  }
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/jupyter/container-by-port/${encodeURIComponent(port)}`);
    ...
    console.warn('[GeoModel Extension] Failed to resolve project from Jupyter port:', error);
  }
...
function inferParameterType(rawType: string = '', datasetType: string = ''): IParameter['type'] {
  const normalizedType = `${rawType} ${datasetType}`.toLowerCase();
  if (normalizedType.includes('external') || /\b(tif|tiff|zip|shp|geojson|csv|nc|json)\b/.test(normalizedType)) {
    return 'file';
  }
  if (normalizedType.includes('bool')) return 'boolean';
  if (normalizedType.includes('int') || normalizedType.includes('real') || normalizedType.includes('double') || normalizedType.includes('float')) {
    return 'number';
  }
  return 'string';
}
...
function parseModelParameters(modelDetail: any): IParameter[] {
  ...
  try {
    ...
    if (Array.isArray(mdlJson.inputs)) {
      mdlJson.inputs.forEach((input: any) => {
        const stateName = input.statename || input.stateName || 'Input';
        const eventName = input.event || input.eventName || input.name || 'Data';
        const children = Array.isArray(input.children) ? input.children : [];
        const optional = input.optional === true || String(input.optional).toLowerCase() === 'true';

        if (children.length > 0) {
          children.forEach((child: any) => {
            const childName = child.eventName || child.name || eventName;
            const childType = child.dataType || child.type || '';
            parameters.push({
              name: `${stateName}.${eventName}.${childName}`,
              sdkName: childName,
              label: childName,
              type: inferParameterType(childType, input.dataType || input.type || ''),
              description: child.description || input.text || input.description || `${stateName} - ${eventName}`,
              required: !optional
            });
          });
        } else {
          parameters.push({
            ...
            label: eventName,
            type: 'file',
            description: input.text || input.description || `${stateName} - ${eventName}`,
            required: !optional
          });
        }
      });
            ...
            for (const state of stateList) {
              ...
              for (const event of events) {
                ...
                if (event.type === 'response') {
                  ...
                  const optional = event.optional === true || String(event.optional).toLowerCase() === 'true';
                  ...
                  for (const param of responseParams) {
                    ...
                    if (dataset?.UdxDeclaration && datasetType !== 'external') {
                      ...
                      for (const node of udxNodes) {
                        parameters.push({
                          ...
                          sdkName: node.name,
                          label: node.name,
                          type: inferParameterType(node.type, datasetType),
  ...
  try {
    ...
    if (methodDetail.params && Array.isArray(methodDetail.params)) {
      methodDetail.params.forEach((param: any) => {
        ...
        if (dataType === 'DataInput') {
        ...
        } else if (dataType === 'DataOutput') {
          paramType = 'string';
          const outputType = typeof pType === 'object' ? (pType.NewFile || pType.ExistingFile || '') : '';
          placeholder = outputType ? `Output filename (${outputType})` : 'Output filename';
          ...
          const optionValues = Array.isArray(pType.OptionList)
            ? pType.OptionList
            : (Array.isArray(pType.OptionList?.options) ? pType.OptionList.options : null);
          if (optionValues) {
            paramType = 'select';
            options = optionValues.map((opt: string) => ({ label: opt, value: opt }));
        ...
        parameters.push({
          ...
          required: param.Optional === false || String(param.Optional).toLowerCase() === 'false',
...
export async function fetchModels(query: string = '', page: number = 1, limit: number = 20): Promise<IPaginatedResult<IModel>> {
  try {
    ...
    const models = (result.data || []).map((m: any) => ({
      ...m,
      id: m.id || m.name,  // Use name as ID when remote ID is missing
      name: m.name,
...
export async function fetchDataMethods(query: string = '', page: number = 1, limit: number = 20): Promise<IPaginatedResult<IDataMethod>> {
  try {
    ...
    const methods = (result.data || []).map((m: any) => ({
      ...m,
      id: m.id || m.name,  // Use name as ID (API queries details by name) when ID is missing
      name: m.name,
      description: m.description || '',
...
export async function fetchMyData(): Promise<ICloudDataItem[]> {
  try {
    const url = `${API_BASE_URL}/jupyter/my-data`;
    const response = await fetch(url, {
      method: 'GET',
      headers: createHeaders(true)
    });
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch My Data: ${response.status}`);
    }
...
export async function fetchProjectDataBindings(projectName: string): Promise<IProjectDataBinding[]> {
  if (!projectName) return [];

  try {
    const url = `${API_BASE_URL}/jupyter/projects/${encodeURIComponent(projectName)}/data-bindings`;
    const response = await fetch(url, {
      method: 'GET',
      headers: createHeaders(true)
    });
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch Project Data bindings: ${response.status}`);
    }

    const result = await response.json();
    return result.dataBindings || [];
  } catch (error) {
    console.error('Error fetching Project Data bindings:', error);
    return [];
  }
}

export async function attachProjectData(projectName: string, dataId: string | number): Promise<{
  binding: IProjectDataBinding | null;
  dataBindings: IProjectDataBinding[];
  materializeWarnings?: any[];
}> {
  const url = `${API_BASE_URL}/jupyter/projects/${encodeURIComponent(projectName)}/data-bindings`;
  const response = await fetch(url, {
    method: 'POST',
  ...
  const result = await response.json();
  if (!response.ok) {
    throw new Error(result.error || `Failed to map data: ${response.status}`);
  }

  return {
    binding: result.binding || null,
    dataBindings: result.dataBindings || [],
    materializeWarnings: result.materializeWarnings || []
  };
}

export async function materializeProjectData(projectName: string, bindingId: string): Promise<{
  binding: IProjectDataBinding | null;
  dataBindings: IProjectDataBinding[];
}> {
  const url = `${API_BASE_URL}/jupyter/projects/${encodeURIComponent(projectName)}/data-bindings/${encodeURIComponent(bindingId)}/materialize`;
  const response = await fetch(url, {
    method: 'POST',
    headers: createHeaders(true)
  });
  ...
  const result = await response.json();
  if (!response.ok) {
    throw new Error(result.error || `Failed to prepare Project Data: ${response.status}`);
  }
 ...
 */
export async function fetchJupyterFiles(subPath: string = '', recursive: boolean = false): Promise<Array<{name: string, type: string, size: number, path?: string}>> {
  try {
    const jupyterBaseUrl = window.location.origin;
    const readDirectory = async (pathPrefix: string, depth: number): Promise<Array<{name: string, type: string, size: number, path?: string}>> => {
      const cleanPath = pathPrefix.replace(/^\/+/, '');
      const contentsUrl = `${jupyterBaseUrl}/api/contents/${encodeURI(cleanPath)}`;
      ...
      const children = await Promise.all(
        mapped
          ...
          .map(item => readDirectory(item.path || item.name, depth + 1))
      );
      const flattened = children.reduce(
        (acc, group) => acc.concat(group),
        [] as Array<{name: string, type: string, size: number, path?: string}>
      );
      return [...mapped, ...flattened];
    };

    return await readDirectory(subPath, 0);
  } catch (error) {
...
export async function fetchModelDetail(modelName: string): Promise<IModel | null> {
  try {
    ...
    return {
      id: modelDetail.sourceId || modelDetail.id || modelName,
      name: modelDetail.name || modelName,
      displayName: modelDetail.displayName || modelDetail.display_name || modelDetail.name || modelName,
      display_name: modelDetail.display_name || modelDetail.displayName || modelDetail.name || modelName,
      ...
      md5: modelDetail.md5,
      parameterParseStatus: modelDetail.parameterParseStatus,
      syncError: modelDetail.syncError || null
    };
  } catch (error) {
